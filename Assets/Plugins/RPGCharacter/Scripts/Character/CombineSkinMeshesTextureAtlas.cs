...
namespace LylekGames.RPGCharacter {
	public class CombineSkinMeshesTextureAtlas : MonoBehaviour {
		...
		public List<Texture2D> texturesMetallic = new List<Texture2D>();
		...
		[HideInInspector]
		public Texture2D specularAtlas;
		[Header("Buttons")]
		...
		public void CombineMeshes() {
			...
			foreach(SkinnedMeshRenderer smr in mySkinnedMeshes) {
				...
				if (smr.sharedMaterials.Length > 0) {
					foreach (Material mat in smr.sharedMaterials) {
						...
						if(mat.HasProperty("_MetallicGlossMap")) {
							...
							Texture2D newSpecMap = new Texture2D(myTexture.width, myTexture.height, textureFormat, isMipMap);
							newSpecMap.SetPixels(0, 0, myTexture.width, myTexture.height, myTexture.GetPixels());

							texturesMetallic.Add(newSpecMap);
						}
						else {
							Debug.LogError (smr.name + " " + mat.name + " does not contain a metallic/specular texture! Atlasing will fail!");
						}
					}
				}
				//GET BONES
				...
				for (int j = 0; j < smr.sharedMesh.subMeshCount; j++) {
					...
					smr.BakeMesh (ciMesh);
					ci.mesh = ciMesh;
					meshIndex[k] = ci.mesh.vertexCount;
			...
			//METALLIC
			specularAtlas = new Texture2D((int)textureAtlasSize, (int)textureAtlasSize, textureFormat, isMipMap);
			specularAtlas.PackTextures(texturesMetallic.ToArray(), 0, (int)textureAtlasSize);
			...
			StandardShaderUtils.ChangeRenderMode (combinedMat, StandardShaderUtils.BlendMode.Cutout);
			combinedMat.SetFloat ("_Mode", 1);
			//ASSIGN OUR TEXTURE ATLAS
			combinedMat.mainTexture = diffuseAtlas;
			combinedMat.SetTexture("_BumpMap", normalsAtlas);
			combinedMat.SetTexture("_MetallicGlossMap", specularAtlas);
			...
			Debug.Log (r.sharedMesh.vertexCount);
		}
		//DISABLE MESH------------------
